"""
Physically Accurate Black Hole Spacetime Visualizations
...
def plot_light_cone_tilting():
    ...
    # Radial positions to show light cones
    radii = [10, 6, 4, 3, 2.5, 2.1, 2.01]
    colors = plt.cm.plasma(np.linspace(0, 0.9, len(radii)))

    for i, r in enumerate(radii):
        if r > RS:
            # Outside horizon: light cones tilt but still allow escape
            # The tilt angle increases as we approach the horizon
            tilt_factor = np.sqrt(1 - RS / r)  # Proper velocity of escape

            # Time coordinate (vertical)
            t_base = (10 - r) * 2

            # Future light cone
            cone_half_angle = 1.0  # In flat space this would be 1 (45 degrees)
            # Asymmetry increases near horizon
...
def plot_penrose_diagram_schwarzschild():
    """
    ...
    where they share a common spacelike slice. However, this bridge pinches
    off too quickly for any signal to traverse it.

    This diagram compactifies infinity so light rays always travel at 45°.
    """
    fig, ax = plt.subplots(figsize=(10, 10))

    # Define the diamond structure
    # Future singularity (r=0, future)
    future_sing = np.array([[0, 4], [-2, 2], [2, 2], [0, 4]])
    # Past singularity (r=0, past)
    past_sing = np.array([[0, -4], [-2, -2], [2, -2], [0, -4]])
    # Region I (our universe)
    region_I = np.array([[2, 2], [4, 0], [2, -2], [0, 0], [2, 2]])
    # Region III (parallel universe)
    ...
    # Add some light cone examples
    # From our universe into black hole
    ax.annotate(
        "",
        xy=(1, 3),
        xytext=(2, 1),
    ...
    # From white hole into our universe
    ax.annotate(
        "",
        xy=(2, 1),
        xytext=(1, -1),
    ...
    # Einstein-Rosen bridge (t=0 slice)
    ax.plot(
...
def plot_eddington_finkelstein():
    """
    ...
    # Ingoing light rays (radial null geodesics)
    v_vals = np.linspace(-10, 20, 15)
    for v in v_vals:
        r = np.linspace(RS, 10, 100)
        # In EF coords: v = constant for ingoing light
        ax.plot([v] * len(r), r, "b-", alpha=0.5, linewidth=1.5)

    # Outgoing light rays
    # For outgoing: v = t + r + 2M ln|r/(2M) - 1|
    ...
    # Example worldline of infalling observer
    v_observer = np.linspace(0, 15, 100)
    r_observer = 8 - 0.4 * v_observer  # Falling inward
    r_observer = np.maximum(r_observer, 0.1)  # Stop at singularity
    ax.plot(
...
def plot_waterfall_model_corrected():
    """
    Corrected Waterfall Model (Gullstrand-Painlevé Coordinates)
    ...
    # Infall velocity magnitude (Gullstrand-Painlevé)
    ...
    # Mask: don't plot inside singularity region
    mask = R > 0.3
...
def plot_kerr_geometry():
    ...
    # Spin parameter (0 < a < 1 for valid black hole)
    a = 0.9  # Near-extremal rotation

    # Horizons
    r_plus = 1 + np.sqrt(1 - a**2)  # Outer event horizon
    r_minus = 1 - np.sqrt(1 - a**2)  # Inner horizon

    # Ergosphere (r = M + √(M² - a²cos²θ))
...
def plot_penrose_kerr():
    ...
    # Region III (inside inner horizon)
    ...
    # Next universe (through the ring!)
...
def plot_visual_appearance_infalling():
    """
    ...
    # Left plot: Apparent position vs observer time
    t_observer = np.linspace(0, 100, 1000)

    # Apparent radius approaches horizon asymptotically
    ...
    # Right plot: Redshift and brightness
    z = np.exp(t_observer / 15) - 1  # Redshift
    brightness = 1 / (1 + z) ** 4  # Gets dim quickly
...
def plot_geodesics_comparison():
    ...
    CORRECTED: Outgoing light rays only escape from well outside the horizon.
    """
    fig, ax = plt.subplots(figsize=(10, 8))

    # Infalling timelike geodesics (massive particles)
    for r0 in [10, 8, 6, 4, 3]:
        # Approximate geodesic (free fall from rest)
        t = np.linspace(0, 40, 200)
        # r(t) for free fall (approximate)
        r = r0 * (1 - 0.02 * t) ** 2
        r = np.maximum(r, 0.1)

        # Only plot until it hits singularity
        valid = r > 0.1
        ax.plot(t[valid], r[valid], "b-", linewidth=2, alpha=0.7)

    # Null geodesics (light rays) - ingoing
    for r0 in [10, 8, 6, 4]:
    ...
    # Null geodesics - outgoing (CORRECTED: only from well outside horizon)
    # These should show the asymptotic behavior as they approach infinity
    for r0 in [3.0, 4.0, 6.0, 8.0]:
        # Only plot for r0 > 2M (outside the horizon)
        r = np.linspace(r0, 10, 100)
        # Tortoise coordinate gives proper time delay
        # For light escaping: t increases logarithmically near horizon
        r_star = r + 2 * RS * np.log(np.abs(r / RS - 1))
        r0_star = r0 + 2 * RS * np.log(np.abs(r0 / RS - 1))
        t = r_star - r0_star  # Relative time

        # Shift to start from reasonable time
        t = t + (r0 - 3) * 5

        ax.plot(t, r, "r--", linewidth=2, alpha=0.7)

    # Circular orbits (timelike geodesics at fixed r)
    ...
    # Important note about outgoing light
    note = (
...
if __name__ == "__main__":
    print("Generating physically accurate black hole visualizations...")
    print("Based on solutions to Einstein's field equations\n")

    plots = [
        ("Light Cone Tilting", plot_light_cone_tilting),
        ("Penrose Diagram (Schwarzschild)", plot_penrose_diagram_schwarzschild),
        ("Eddington-Finkelstein Coordinates", plot_eddington_finkelstein),
        ("Corrected Waterfall Model", plot_waterfall_model_corrected),
        ("Kerr Black Hole Structure", plot_kerr_geometry),
        ("Penrose Diagram (Kerr)", plot_penrose_kerr),
        ("Visual Appearance of Infalling Objects", plot_visual_appearance_infalling),
        ("Geodesics Comparison (CORRECTED)", plot_geodesics_comparison),
    ]
    ...
    print("7. CORRECTED: Outgoing light only escapes from outside the horizon!")
